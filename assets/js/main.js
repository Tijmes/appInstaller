...
mc.on("panend", function(ev) {
	...
	if(lv.topPanelOpen == 1){
		...
		if(lv.uVelocity > 2 || lv.uVelocity < -2){
			...
			if(lv.lockPanDir == 1){
				...
				TweenMax.to('#themesWrap', 0.3, {x:lv.panDestX+'%', ease:lv.navEase});
				//
				//
	...
	if(lv.uVelocity > 2 || lv.uVelocity < -2){
		...
		if(lv.lockPanDir == 1){
			...
			TweenMax.to('#secWrap', 0.3, {x:lv.panDestX+'%', ease:lv.navEase});
			//
...
function handleThemeControls(){
	...
	$('.playicon').css({'border-color':'rgb(255,255,255)'}).removeClass('itemIconActive');
